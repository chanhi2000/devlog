...
meta:
  ...
  - property: og:url
    content: https://chanhi2000.github.io/explore/crashcourse/paul-hudson-100-days-of-swiftui/41.html
---
...
Let’s start simple and work our way up: make a new SwiftUI view called <FontIcon icon="iconfont icon-file"/> `MissionView.swift`. Initially this will just have a `mission` property so that we can show the mission badge and description, but shortly we’ll add more to it.
...
Put this code into <FontIcon icon="iconfont icon-file"/> `MissionView.swift` now:
